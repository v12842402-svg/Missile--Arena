...
function shootGun() {
  ...
  if (ammo <= 0) {
    ...
    setMessage(
      "❌ مهمات تمام شده! از فروشگاه دوباره مهمات بخر."
    );
...
function shootMissile() {
  ...
  if (ammo <= 0) {
    ...
    setMessage(
      "❌ موشک تمام شده! از فروشگاه دوباره تهیه کن."
    );

    return;
  }

  player.missileAmmo[missile]--;
...
function enemyAttack() {
  ...
  // Vest absorbs the hit

  if (
    player.currentVestDurability > 0
  ) {

    player.currentVestDurability =
      Math.max(
        0,
        player.currentVestDurability - 5
      );
...
function fixPlayerData() {
  ...
  if (!player.ownedMissiles) {
    player.ownedMissiles = {
      "Scud-B": true
    };
  }

  if (!player.ownedDefense) {
    player.ownedDefense = {
      25: true
    };
  }

  if (!player.ownedVests) {
    player.ownedVests = {
      25: true
    };
  }
  ...
  if (!player.missileAmmo) {
    player.missileAmmo = {};
  }

  if (
    player.ammo["AK-47"] === undefined
  ) {
    player.ammo["AK-47"] = 10;
  }

  if (
    player.missileAmmo["Scud-B"] === undefined
  ) {
    player.missileAmmo["Scud-B"] = 2;
  }

  if (
    player.currentVestDurability === undefined
  ) {
    player.currentVestDurability =
      player.currentVest || 25;
  }
...
function initializeGame() {
  ...
  console.log(
    "🚀 Missile Arena loaded successfully!"
  );
}
